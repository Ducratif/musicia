...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('cover')
    .setDescription('Cover Song (POST /Cover)')
    ...
    .addIntegerOption(o => o.setName('pitch').setDescription('Pitch (default 0)').setRequired(false))
  ...
  async execute(interaction, ctx) {
    ...
    const r = await ctx.api.cover({ audio_url, voice_id, pitch, webhook_url: '' });
    if (!r.ok || r.json?.success === false) {
      return interaction.editReply({ embeds: [errorEmbed('API Error', fmtApiError(r.status, r.json))] });
    }

    const task = r.json;
    const maxMs = Math.min(12 * 60 * 1000, Math.max(60_000, (Number(task.eta) || 60) * 1000 * 3));

    const done = await pollUntilDone(
      () => ctx.api.getById({ conversionType: ConversionTypes.COVER, task_id: task.task_id }),
      { intervalMs: 4000, maxMs }
    );

    if (done.timeout) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setTitle('Cover — Timeout').setDescription(`Utilise \`/status\` type COVER id \`${task.task_id}\`.`)]
      });
    }
    ...
    if (String(conv.status).toUpperCase() !== 'COMPLETED' || !url) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setTitle('Cover — Erreur').setDescription(`Status: \`${conv.status ?? 'N/A'}\`\nMsg: ${conv.status_msg ?? 'N/A'}`)]
      });
    }

    const out = await attachOrLink(url, 'cover.mp3', ctx.limits.maxUploadMb);
    const files = out.kind === 'file' ? [out.file] : [];

    const e = new EmbedBuilder()
      .setTitle('✅ Cover — Résultat')
      .setDescription([
