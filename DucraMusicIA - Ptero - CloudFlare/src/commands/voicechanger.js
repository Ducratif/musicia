...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('voicechanger')
    .setDescription('Voice Changer (POST /VoiceChanger)')
    .addStringOption(o => o.setName('voice_id').setDescription('Voice ID').setRequired(true))
    ...
    .addIntegerOption(o => o.setName('remove_background').setDescription('0/1 (optionnel)').setRequired(false))
    .addIntegerOption(o => o.setName('pitch').setDescription('Pitch (ex: -12..+12)').setRequired(false))
  ...
  async execute(interaction, ctx) {
    ...
    const voice_id = interaction.options.getString('voice_id', true);
    const remove_background = interaction.options.getInteger('remove_background') ?? 0;
    const pitch = interaction.options.getInteger('pitch') ?? 0;
    ...
    const r = await ctx.api.voiceChanger({ audio_url, voice_id, remove_background, pitch, webhook_url: '' });
    ...
    const done = await pollUntilDone(
      () => ctx.api.getById({ conversionType: ConversionTypes.VOICE_CONVERSION, task_id: task.task_id }),
      { intervalMs: 4000, maxMs }
    );

    if (done.timeout) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setTitle('VoiceChanger — Timeout').setDescription(`Utilise \`/status\` type VOICE_CONVERSION id \`${task.task_id}\`.`)]
      });
    }
    ...
    if (String(conv.status).toUpperCase() !== 'COMPLETED' || !url) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setTitle('VoiceChanger — Erreur').setDescription(`Status: \`${conv.status ?? 'N/A'}\`\nMsg: ${conv.status_msg ?? 'N/A'}`)]
      });
    }

    const out = await attachOrLink(url, 'voicechanger.mp3', ctx.limits.maxUploadMb);
    const files = out.kind === 'file' ? [out.file] : [];

    const e = new EmbedBuilder()
      .setTitle('✅ VoiceChanger — Résultat')
      .setDescription([
